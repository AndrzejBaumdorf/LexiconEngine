...
class LexiconDatabase:
	...
	def _repair_legacy_schema(self) -> None:
		"""Recreate the local database if an older schema lacks a valid parent key."""
		words_table = self.connection.execute("PRAGMA table_info(words)").fetchall()
		id_column = next((row for row in words_table if row[1] == "id"), None)
		if id_column is None or id_column[5] == 1:
			return
		...
		self.connection.commit()
		print("旧形式のDBを検出したため、スキーマを再作成しました。")
	...
	def create_japanese_to_english_question(self) -> Question:
			word_count = self.connection.execute("SELECT COUNT(*) FROM words").fetchone()[0]
			if word_count < 4:
				raise ValueError("4択には4語以上の単語データが必要です")
			target = self.connection.execute(
